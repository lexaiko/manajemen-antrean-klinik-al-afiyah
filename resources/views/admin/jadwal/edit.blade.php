<x-app-layout>
    <x-layout>
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="{{ route('admin.jadwal.edit', ['id' => $jadwal->id]) }}"
        ...
        <form action="{{ route('admin.jadwal.update', ['id' => $jadwal->id]) }}" method="post"
            enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="grid gap-2 sm:grid-cols-2 sm:gap-2">
                <div class="sm:col-span-2">
                    <label for="pegawai_id" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama
                        Pegawai</label>
                    <select name="pegawai_id" id="pegawai_id"
                        ...
                        @foreach ($tenagaMedis as $tm)
                            <option value="{{ $tm->id }}" {{ $jadwal->pegawai_id == $tm->id ? 'selected' : '' }}>
                                {{ $tm->name }} - {{ $tm->roles->pluck('name')->join(', ') }}
                            </option>
                        @endforeach
                    </select>
                    @error('pegawai_id')
