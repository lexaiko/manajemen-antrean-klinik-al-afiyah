        ...
        <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg">
            <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 p-4">
                ...
                <div
                    ...
                    <div
                        class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                        <a href="{{ route('admin.tenagamedis.create') }}"
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-3">Jenis Tenaga</th>
                            <th scope="col" class="px-4 py-3">Spesialisasi</th>
                            <th scope="col" class="px-4 py-3 text-right">Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($tenagaMedis as $user)
                            <tr class="border-b dark:border-gray-700">
                                <td class="px-4 py-3">
                                    {{ ($tenagaMedis->currentPage() - 1) * $tenagaMedis->perPage() + $loop->iteration }}</td>
                                <td class="px-4 py-3">{{ $user->user->name }}</td>
                                <td class="px-4 py-3">{{ $user->nip }}</td>
                                <td class="px-4 py-3">{{ $user->jenis_tenaga }}</td>
                                <td class="px-4 py-3">{{ $user->spesialisasi ?? '-' }}</td>
                                <td class="px-4 py-3 flex items-center justify-end">
                                    ...
                                    <div id="actionsDropdown-{{ $user->id }}"
                                        ...
                                        <ul class="py-1 text-sm text-gray-700 dark:text-gray-200"
                                            ...
                                            <li>
                                                <a href="{{ route('admin.tenagamedis.edit', $user->id) }}"
                                                    class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Edit</a>
                                            </li>
                                        </ul>
                                        <div class="py-1">
                                            <form action="{{ route('admin.tenagamedis.destroy', $user->id) }}"
                                                method="POST"
            ...
            <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4"
                ...
                <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                    ...
                    <span class="font-semibold text-gray-900 dark:text-white">
                        @if ($tenagaMedis->count())
                            {{ $tenagaMedis->firstItem() }}-{{ $tenagaMedis->lastItem() }}
                        @else
                            0-0
                        @endif
                    </span>
                    dari
                    <span class="font-semibold text-gray-900 dark:text-white">{{ $tenagaMedis->total() }}</span>
                </span>
                <div>
                    {{ $tenagaMedis->links() }} <!-- Menampilkan link navigasi halaman -->
                </div>
            </nav>
