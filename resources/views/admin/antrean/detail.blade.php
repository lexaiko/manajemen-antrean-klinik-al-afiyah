    ...
    <x-layout>
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="/admin/user"
                            class="ms-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ms-2 dark:text-gray-400 dark:hover:text-white">Pegawai</a>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg">
            ...
            @foreach ($antrian as $user)
                <div class="border border-gray-200 rounded p-4 mb-4 shadow-sm bg-white dark:bg-gray-800">
                    <div class="mb-2"><strong>No:</strong>
                        {{ ($antrian->currentPage() - 1) * $antrian->perPage() + $loop->iteration }}</div>
                    <div class="mb-2"><strong>Nama Pasien:</strong> {{ $user->name }}</div>
                    <div class="mb-2"><strong>Email:</strong> {{ $user->email }}</div>
                    <div class="mb-2"><strong>Jenis Kelamin:</strong>
                        {{ $user->jenis_kelamin == 'L' ? 'Laki-laki' : 'Perempuan' }}</div>
                    <div class="mb-2"><strong>Keluhan:</strong> {{ $user->role_id }}</div>
                    <div class="mb-2"><strong>Nomor Antrian:</strong> {{ $user->nomor_antrian ?? '-' }}</div>
                    <div class="mb-2"><strong>Alamat Pasien:</strong> {{ $user->alamat ?? '-' }}</div>
                    <div class="mb-2"><strong>Tanggal Kunjungan:</strong> {{ $user->tanggal_kunjungan ?? '-' }}</div>
                    <div class="mb-2"><strong>Nomor WhatsApp:</strong> {{ $user->nomor_whatsapp ?? '-' }}</div>
