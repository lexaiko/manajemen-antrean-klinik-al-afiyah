<x-app-layout>
    <x-layout>
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="/admin/role" class="ms-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ms-2 dark:text-gray-400 dark:hover:text-white">Peran</a>
                    </div>
                </li>
            </ol>
        </nav>
        <div class="flex justify-between items-center mb-4">
            <h1 class="py-2 text-xl font-bold text-gray-900 dark:text-white">Daftar Peran</h1>
        </div>
                ...
                <div class="flex items-center w-full">
                    <form action="" method="GET" class="flex items-center w-full max-w-md space-x-2">
                        <div class="relative flex-grow">
                            ...
                            <input type="text" name="search" placeholder="Cari peran..."
                    ...
                    @forelse ($roles as $role)
                        <tr class="border-b dark:border-gray-700">
                            ...
                            <td class="px-4 py-3 flex items-center justify-end">
                                <button id="actionsDropdownButton-{{ $role->id }}" data-dropdown-toggle="actionsDropdown-{{ $role->id }}" class="inline-flex items-center p-2 text-sm font-medium text-center text-gray-500 bg-white rounded-lg border border-gray-300 hover:bg-gray-100 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 focus:outline-none focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700" type="button">
                                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" />
                                    </svg>
                                </button>
                                <!-- Dropdown menu -->
                                <div id="actionsDropdown-{{ $role->id }}" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                    <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="actionsDropdownButton-{{ $role->id }}">
                                        <li>
                                            <a href="{{ route('admin.role.edit', $role->id) }}" class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Edit</a>
                                        </li>
                                    </ul>
                                    <div class="py-1">
                                        <form action="{{ route('admin.role.destroy', $role->id) }}" method="POST" onsubmit="return confirm('Apakah anda yakin ingin menghapus role ini?');">
                                            @csrf
                                            @method('DELETE')
        ...
        <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4"
                ...
                <div>
                    {{ $roles->links() }} <!-- Menampilkan link navigasi halaman -->
                </div>
            </nav>
        </div>
