        ...
        <div
            ...
            <div class="w-full">
                ...
                <div class="flex flex-col md:flex-row md:justify-between md:items-center mb-4 gap-2">
                    <form method="GET" action="{{ route('antrean.index') }}"
                        ...
                        <select id="filter" name="filter" onchange="this.form.submit()"
                            ...
                            @foreach ($polis as $poli)
                                <option value="{{ $poli->id }}" {{ request('filter') == $poli->id ? 'selected' : '' }}>
                                    {{ $poli->nama_poli }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <button type="button" class="text-white bg-green-600 hover:bg-green-600 focus:ring-4 focus:ring-green-200 font-medium rounded-lg text-sm px-2 py-1.5 md:items-center">
                        <a href="{{ route('antrean.registrasi') }}" class="">Ambil Nomor Antrean</a>
